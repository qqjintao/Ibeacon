		...
		<div id="app" style="width: 50%;">
			<el-form :model="form" :rules="rules2" ref="form" label-width="120px">
				<el-form-item label="报警时间：" prop="alarmTime">
					<span v-if="form.seen">${fence.alarmTime}</span>
				...
				<el-form-item label="报警声音：" prop="alarmVoice">
					<span v-if="form.seen">${fence.alarmVoice}</span>
				...
				<el-form-item label="发送手机：" prop="phone">
					<span v-if="form.seen">${fence.phone}</span>
		...
		var Main = {
			data() {
				...
				return {
					form: {
						seen: true,
						alarmTime: "${fence.alarmTime}",
						alarmVoice: "${fence.alarmVoice}",
						isSend: "${fence.isSend}",
						phone: "${fence.phone}",
						options: [{
								value: 'Y',
				...
				submitForm(form) {
					this.$refs[form].validate((valid) => {
						if(valid) {
							...
							$.ajax({
								type: 'post',
								url: '${base}/fence/fenceUpdate.jhtml',
								data: {
									"id": '${fence.id}',
									"alarmTime": alarmTime,
								...
								success: function(date) {
									if(date) {
										...
										setTimeout(function() {
											window.location.href = "${base}/fence/index.jhtml";
