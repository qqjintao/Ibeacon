		...
		<div id="app">
						...
						<tr>
							<td>
								<el-form-item label="设备名称：" prop="name">
									<el-input type="text" v-model="form.name" auto-complete="off" name="name"></el-input>
								</el-form-item>
							</td>
							<td>
								<el-form-item label="设备位置：" prop="address">
									<el-input type="text" v-model="form.address" auto-complete="off" name="address"></el-input>
								</el-form-item>
							</td>
							<td>
							...
							<td>
								<el-form-item label-width="30px">
									<el-button type="primary" @click="updata" round>设置电子围栏模板</el-button>
								</el-form-item>
							</td>
						</tr>
					</tbody>
			...
			<el-table stripe ref="multipleTable" :data="tableData" tooltip-effect="dark" style="width: 100%" @selection-change="handleSelectionChange">
				...
				<el-table-column label="操作" width="180">
					<template slot-scope="scope">
						<el-button size="mini" @click="handleEdit(scope.row.id)">设置电子围栏</el-button>
					</template>
				</el-table-column>
			</el-table>
			<div class="block">
				...
				data() {
					return {
						...
						methods: {
							...
							updata() {
								window.location.href = "${base}/fence/updata.jhtml";
							},
							handleEdit(row) {
								window.location.href = "${base}/fence/fenceEdit.jhtml?id=" + row;
							},
							handleCurrentChange(val) {
								var name = this.form.name;
								var address = this.form.address;
								window.location.href = "${base}/fence/index.jhtml?name=" + name + "&address=" + address + "&pageNumber=" + val;
							},
							submitForm(form) {
								this.$refs[form].validate((valid) => {
									if(valid) {
										...
										var address = this.form.address;
										var that = this;
										window.location.href = "${base}/fence/index.jhtml?name=" + name + "&address=" + address;
									} else {
