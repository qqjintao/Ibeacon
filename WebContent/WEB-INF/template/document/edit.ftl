		...
		<div id="app" style="width: 50%;">
			<el-form :model="form" :rules="rules2" ref="form" label-width="100px" class="demo-ruleForm">
				<el-form-item label="文档名称：" prop="name">
					<span v-if="form.seen">${document.name}</span>
				...
				<el-form-item label="文档地址：" prop="loadAddress">
					<span v-if="form.seen">${document.loadAddress}</span>
					<el-input type="text" v-model="form.loadAddress" auto-complete="off" v-if="!form.seen"></el-input>
				</el-form-item>
			...
			data() {
				...
				return {
					form: {
						seen: true,
						name: "${document.name}",
						loadAddress: "${document.loadAddress}",
				...
				submitForm(form) {
					this.$refs[form].validate((valid) => {
						if(valid) {
							...
							$.ajax({
								type: 'post',
								url: '${base}/document/documentUpdate.jhtml',
								data: {
									"id": '${document.id}',
									"createDate": '${document.createDate}',
									"createName": '${document.createName}',
									"name": name,
