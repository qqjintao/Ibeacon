	...
	<body>
		<div id="app">
							...
							<td>
								<center>
									<el-form-item label-width="30px">
										<el-button type="primary" @click="add" round>新增操作文档</el-button>
										...
										<el-upload class="upload-demo" style="display:inline" action="${base}/import/uploadExecl.jhtml?downtype=document" :before-upload="beforeAvatarUpload" accept="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet, application/vnd.ms-excel" multiple :limit="3" :on-exceed="handleExceed">
			...
			<el-table stripe ref="multipleTable" :data="tableData" tooltip-effect="dark" style="width: 100%" @selection-change="handleSelectionChange">
				...
				<el-table-column prop="loadAddress" label="下载的地址" show-overflow-tooltip>
				</el-table-column>
				...
				<el-table-column label="操作" width="270px">
					<template slot-scope="scope">
						...
						<el-button size="mini" @click="download(scope.row.loadAddress)">下载</el-button>
		...
		var Main = {
				data() {
					return {
						...
						tableData: [
							[#list page.getContent() as documents] {
								id: "${documents.id}",
								createDate: '${documents.createDate}',
								createName: '${documents.createName}',
								modifyDate: '${documents.modifyDate}',
								modifyName: '${documents.modifyName}',
								loadAddress: '${documents.loadAddress}',
								name: '${documents.name}',
						...
						methods: {
							...
							add() {
								window.location.href = "${base}/document/documentAdd.jhtml";
							},
							handleEdit(row) {
								window.location.href = "${base}/document/documentEdit.jhtml?id=" + row;
							},
							exportDate() {
								var name = this.form.name;
								window.location.href = "${base}/document/export.jhtml?name=" + name + "&pageNumber=" + "${page.getPageNumber()}";
							},
							download(loadAddress) {
								window.location.href = loadAddress;
							},
							...
							exportAll() {
								window.location.href = "${base}/document/export.jhtml";
							},
							deleteSelection() {
								let ids = [];
								...
								window.location.href = "${base}/document/deleteSelection.jhtml?ids=" + ids;
							},
							handleCurrentChange(val) {
								var name = this.form.name;
								window.location.href = "${base}/document/index.jhtml?name=" + name + "&pageNumber=" + val;
							},
							handleDelete(index, rows, id) {
								this.$confirm('确认关闭？')
									.then(_ => {
										...
										$.ajax({
											type: 'post',
											url: '${base}/document/documentDelete.jhtml',
											data: {
												"id": id,
											},
											dataType: "json",
							...
							submitForm(form) {
								this.$refs[form].validate((valid) => {
									if(valid) {
										var name = this.form.name;
										var that = this;
										window.location.href = "${base}/document/index.jhtml?name=" + name;
									} else {
